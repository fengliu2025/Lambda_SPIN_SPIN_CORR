...
public : 
	...
	TH2D *h2D_NK0s_NGoodK0s; 
	TH2D *h2D_NK0s_PtDiff[1];
	TH2D *h2D_NK0s_RapidityDiff[1];
	TH2D *h2D_NK0s_PhiDiff[1];
	TH2D *h2D_NK0s_DeltaR[1];

	TH2D *h2D_NK0s_pairMass[1];
	TH2D *h2D_pairPt_pairMass[1];
	TH2D *h2D_pairEta_pairMass[1];
	TH2D *h2D_pairPhi_pairMass[1];
	TH2D *h2D_pairDecayL_pairMass[1];
	TH2D *h2D_p1Pt_pairMass[1];
	TH2D *h2D_p2Pt_pairMass[1];
	...
	void InitHitogram();
	void Fill_QAplots();
	void Fill_QAplots(std::vector<int> GoodK0sFlag);
	void Fill_PairPlots(TLorentzVector *v1,TLorentzVector *v2, int pair_type);
	void Fill_NK0s_NGoodK0s(int NGoodK0s);
	void Reset();
...
void ntp_K0s_Histogram::InitHitogram(){
	...
	h1D_NK0s    = new TH1D("h1D_NK0s","h1D_NK0s",10,-1,9);
	h1D_TrigID     = new TH1D("h1D_TrigID","h1D_TrigID",6,0.5,6.5);

	h2D_NK0s_Ntrks = new TH2D("h2D_NK0s_Ntrks","h2D_NK0s_Ntrks",10,-1,9,100,-1,99);
	...
	h2D_NK0s_NGoodK0s = new TH2D("h2D_NK0s_NGoodK0s","h2D_NK0s_NGoodK0s",10,0.5,10.5,11,-0.5,10.5); 
	
	for(int i =0 ; i < 1 ; i ++){
		h2D_NK0s_PtDiff[i]        = new TH2D(Form("h2D_NK0s_PtDiff_%d",i),Form("h2D_NK0s_PtDiff_%d",i),10,0.5,10.5,100,-10,10 );
		h2D_NK0s_RapidityDiff[i]  = new TH2D(Form("h2D_NK0s_RapidityDiff_%d",i),Form("h2D_NK0s_RapidityDiff_%d",i),10,0.5,10.5,100,-5,5);
		h2D_NK0s_PhiDiff[i]       = new TH2D(Form("h2D_NK0s_PhiDiff_%d",i),Form("h2D_NK0s_PhiDiff_%d",i),10,0.5,10.5,200,-2*TMath::Pi(),2*TMath::Pi());
		h2D_NK0s_DeltaR[i]        = new TH2D(Form("h2D_NK0s_DeltaR_%d",i),Form("h2D_NK0s_DeltaR_%d",i),10,0.5,10.5,100,0,4 );
...
void ntp_K0s_Histogram::Fill_QAplots(){
		h1D_Vz     				->Fill(Reader->Vz);
		h1D_NK0s     		->Fill(Reader->NK0s);
		...
		h2D_NK0s_Ntrks->Fill(Reader->NK0s,Reader->mNTrks);
		...
		for(int i =0 ; i < Reader->NK0s;i++){
			...
			h1D_p1_nSigmaPion ->Fill(Reader->p1_nSigmaPion[i]);
...
void ntp_K0s_Histogram::Fill_QAplots(std::vector<int> GoodK0sFlag){
		h1D_Vz     				->Fill(Reader->Vz);
		h1D_NK0s     		->Fill(Reader->NK0s);
		for(int i =0; i < Reader->mNTrigs;i++){
			if(Reader->mTrigId[i] == 910001 )     {h1D_TrigID->Fill(1);}
...
void ntp_K0s_Histogram::Fill_PairPlots(TLorentzVector *v1,TLorentzVector *v2, int pair_type){
	...
	h2D_NK0s_PtDiff[pair_type]->Fill(Reader->NK0s,delta_Pt);
	h2D_NK0s_RapidityDiff[pair_type]->Fill(Reader->NK0s,delta_Rapidity);
	h2D_NK0s_PhiDiff[pair_type]->Fill(Reader->NK0s,delta_Phi);
	h2D_NK0s_DeltaR[pair_type]->Fill(Reader->NK0s,deltaR);
...
void ntp_K0s_Histogram::Fill_NK0s_NGoodK0s(int NGoodK0s){
	h2D_NK0s_NGoodK0s->Fill(Reader->NK0s,NGoodK0s);
}
...
void ntp_K0s_Histogram::Reset(){
	...
	for(int i =0 ; i <1 ; i++){
		h2D_NK0s_PtDiff[i]->Reset("ICES");
		h2D_NK0s_RapidityDiff[i]->Reset("ICES");
		h2D_NK0s_PhiDiff[i]->Reset("ICES");
		h2D_NK0s_DeltaR[i]->Reset("ICES");
	}
	
	for(int i=0 ;i <1 ;i++){
		h2D_NK0s_pairMass[i]->Reset("ICES");
...
void ntp_K0s_Histogram::WriteAll(){
	...
	h2D_NK0s_NGoodK0s->Write(); 
	for(int i =0 ; i <1 ; i++){
		h2D_NK0s_PtDiff[i]->Write();
		h2D_NK0s_RapidityDiff[i]->Write();
		h2D_NK0s_PhiDiff[i]->Write();
		h2D_NK0s_DeltaR[i]->Write();
	}
	
	for(int i=0 ;i <1 ;i++){
		h2D_NK0s_pairMass[i]->Write();
