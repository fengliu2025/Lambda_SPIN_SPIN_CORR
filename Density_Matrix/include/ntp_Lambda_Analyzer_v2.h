...
void ntp_Lambda_Analyzer::FindCounterparts(std::vector<TLorentzVector> *Lambda_counterpart,std::vector<TLorentzVector> *proton_counterpart,std::vector<TLorentzVector> *pion_counterpart,double rapidity,int I_LAMBDA,int I_EVENT,int I_FILE){
	...
	for(int i_mixfile=0; i_mixfile < MixTreeReader->InputFiles[I_FILE].size() ; i_mixfile ++){
		...
		if(!f_mixevent){
			std::cout<<"Can noyt open the mixevent file:"<<MixEvent_Reader->InputFiles[i_mixfile] <<", Skip this." <<std::endl;
			continue;
		...
		for(int iLamCon = MixTreeReader->Start_Index[I_LAMBDA]; iLamCon < MixTreeReader->End_Index[I_LAMBDA]; iLamCon ++){
			MixEvent_Reader->fChain->GetEntry( MixTreeReader->CounterPart_EventIndex[iLamCon] );
			...
			int isGoodCounterPart = LambdaSelecter->IsGoodLambdaCounterpart(MixEvent_Reader->pair_pt[0], SameEvent_Reader->pair_pt[I_LAMBDA],
																	  v.Rapidity(),rapidity,
																	  MixEvent_Reader->pair_phi[0], SameEvent_Reader->pair_phi[I_LAMBDA],
																	  MixEvent_Reader->pair_charge[0], SameEvent_Reader->pair_charge[I_LAMBDA],
																	  MixEvent_Reader->p1_ch[0], SameEvent_Reader->p1_ch[I_LAMBDA] );
